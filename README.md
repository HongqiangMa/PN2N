...
![PN2N example result](PN2N_show.jpg)
...
Photon-limited microscopy is constrained by the finite photon budget required to preserve sample viability, photostability, and imaging speed. Although denoising methods have advanced substantially, many require high-quality ground-truth data or use self-supervised strategies that discard spatial information through pixel masking or rely on noise assumptions that are mismatched to photon-counting measurements. Here we introduce PN2N, a physics-informed self-supervised denoising model that uses Poisson thinning to generate statistically independent training pairs from a single raw acquisition. By binomially splitting detected photon counts, PN2N preserves photon-counting statistics and full spatial support, enabling self-supervised learning without repeated measurements, masking-based information removal, or synthetic noise approximation. Across simulated resolution targets and microtubule networks, as well as experimental fluorescence and brightfield microscopy datasets, PN2N more faithfully recovered weak signals and preserved fine spatial detail compared with representative denoising methods. PN2N further largely improved downstream quantitative phase-image reconstruction, demonstrating that its benefits extend beyond visual restoration to quantitative computational imaging under low-dose conditions. Its lightweight network implementation also enabled substantially faster convergence than other trainable self-supervised methods. These results establish PN2N as a practical, physics-grounded framework for accurate and photon-efficient denoising in microscopy.
...
PN2N is provided for academic and non-commercial research use only. Commercial use requires a separate license from the authors.
